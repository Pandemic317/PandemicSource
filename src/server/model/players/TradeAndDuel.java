...
public class TradeAndDuel{
	...
	**/
	...
	public void requestTrade(int id){
		...
		if(c.npcKills < 11) {
			c.sendMessage("You need at least 10 monster kills in order to trade! You currently have <col=800000000>"+ c.npcKills +"</col> kills.");
			return;
		}
		if(c.playerRights == 2) {
			c.sendMessage("Administrator can't trade players.");
			return;
		}
		if(c.ironGambler >= 1) {
			c.sendMessage("You can't trade players as an Iron Gambler.");
			return;
		}
		try {
