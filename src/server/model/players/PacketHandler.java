...
import server.model.players.packets.AuctionSearch;
...
import server.model.players.packets.GambleResponse;
...
public class PacketHandler {
	 ...
	 * packetId[packetType]; if(p != null) { try { //System.out.println(
	 * "packet: " + packetType); p.processPacket(c, packetType, packetSize); }
	 * catch(Exception e) { e.printStackTrace(); c.disconnected = true; //this
	 * is for when the isaac gets out of wack the player will d } } else {
	...
	public static void processPacket(Client c, int packetType, int packetSize) {
		...
		if (p != null && packetType > 0 && packetType < 257 && packetType == c.packetType
			...
			c.disconnected = true;
			System.out.println(
					c.playerName + "is sending invalid PacketType: " + packetType + ". PacketSize: " + packetSize);
		}
	}
