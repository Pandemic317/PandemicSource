...
import server.model.players.Content.familiar.FamiliarAction;
...
public class Client extends Player {
	...
	public int timer;
	private GambleInterface gambleInterface = new GambleInterface(this);
	...
	private AuctionInterface auctionHouse = new AuctionInterface(this);
	...
	public void getYellTag() {
		if (customYell == true)
			return;
		/***
		 * An example of how names can be used if
		 * (playerName.equalsIgnoreCase("Infliction")) { yellTag =
		 * defaultYellTag[6]; return; }
		 */
		/***
		 * An example of how ints besides playerrights can be used if (veteren
		 * == 1) { yellTag = defaultYellTag[7]; return; }
		 */
	...
	public boolean checkEmpty44(Player player) {
		if ((getItems().freeSlots() == 28 && playerEquipment[playerHat] == -1) && (playerEquipment[playerCape] == -1)
	 ...
	 * != 11951 && !getPA().inPitsWait() && !inPits) { sendMessage(
	 * "You need to wear a snowball first.."); return; }
	 * 
	...
	/*public void DT() {
		for (int j = 0; j < Server.playerHandler.players.length; j++) {
			if (Server.playerHandler.players[j] != null) {
				...
				// c2.sendMessage("<shad=7>"+playerName+" has just completed the
	...
	/*
	 * public void thrones() { if (playerRights < 3) { sendMessage(
	 * "You don't have the rights to sit here."); return; } else { if
	 * (objectType == 13671) { // getPA().movePlayer(absX, absY, 0);
	...
	public void BestItem2(int ItemsContained) {
		...
		if (!isSkulled && ItemsContained > 2
				&& (WillKeepAmt1 + WillKeepAmt2 < 3 || (prayerActive[10] && WillKeepAmt1 + WillKeepAmt2 < 4))) {
	...
	int[] DragonImpRare = { 990, 829, 391, 385, 564, 1249, 3052, 1303, 808, 1602, 1917, 1971, 1514, 1319, 1275, 3024,
			1606, 454, 560, 563, 561, 1373, 1127, 5300, 2440, 1604, 1452, 1149, 830, 1079, 886, 1608, 565, 554, 1079,
			1093, 1127, 1432, 1333, 1185, 2366, 1249, 1303, 385, 3024, 564, 3052, 561, 1275, 5300, 989, 19337, 19341,
			19336, 19338, 19339, 19340, 19342, 19343, 19345, 19344 };

	public int DragonImpRare() {
		return DragonImpRare[(int) (Math.random() * DragonImpRare.length)];
	}

	int[] DragonImpCommon = { 989, 829, 391, 385, 564, 1249, 3052, 1303, 808, 1602, 1917, 1971, 1514, 1319, 1275, 3024,
			1606, 454, 560, 563, 561, 1373, 1127, 5300, 2440, 1604, 1452, 1149, 830, 1079, 886, 1608, 565, 554, 1079,
			1093, 1127, 1432, 1333, 1185, 2366, 1249, 1303, 385, 3024, 564, 3052, 561, 1275, 5300 };
	...
	int[] randomreward2 = { 4151, 4151, 4131, 4131, 4131, 4093, 4093, 4093, 4087, 4087, 4087, 2577, 2577, 2577, 5525,
			5525, 5525, 4734, 4720, 4730, 4745, 4712, 4753, 4755, 4738, 6524, 6524, 6524, 6524, 6524, 6524, 6585, 6585,
			6585, 6585, 6585, 6585, 6585, 6585, 6585, 6585, 6731, 6731, 6731, 6731, 6731, 6733, 6733, 6733, 6733, 6733,
			6733, 6733, 6733, 6735, 6735, 6735, 6735, 6735, 6735, 6735, 6737, 6737, 6737, 6737, 6737, 6737, 6737, 6739,
			6739, 6739, 6739, 6739, 6739, 6739, 6739, 6739, 6739, 6739, 6739, 15312, 15312, 15312, 15312, 15312, 15312,
			15312, 15312, 15312, 15312, 15312, 15312, 15315, 15315, 11690, 11690, 11690, 11690, 11690, 11690, 11690,
			11690, 11690, 11690, 11690, 11690, 11690, 3140, 3140, 3140, 3140, 3140, 3140, 3140, 3140 };
	...
	public void healSummon(int heal) {
		...
		if (futurehp > getPA().getLevelForXP(playerXP[3])) {
			...
			// sendMessage("You didn't get healed because you've alredy got Full
			// HP!");
	...
	public void Hack3rsAutoSaver() {
		if (!disconnected) {
			...
			EventManager.getSingleton().addEvent(new Event() {
				public void execute(EventContainer container) {
					sendMessage("<shad=132833>Pandemic: Game saved successfully for everyone!");
					// System.out.println("Automaticly Saved Game for
					// everyone!");
					saveCharacter = true;
					SaveGame();
	 ...
	 * "SERVER SHUT DOWN BY EVENT shutDown();");
	 * 
	 * //saveGameDelay = System.currentTimeMillis();
	 * 
	 * //} //} } }, 7200000); }
	...
	public void trade11() {
		if (playerRights == 7 || playerRights == 1 || playerRights == 2) {
			Server.getTaskScheduler().addEvent(new Task(1, true) {
				public void staffLogin() {
			staffLogin ++;
			if (disconnected) {
				String playerWrite = "The total of " + playerName + " is: "+ staffLogin+" seconds.";
			Connection.staffLogin(playerWrite);
				this.stop();
			}
		}
		...
		if (hasFollower == 6870) {
			Server.getTaskScheduler().addEvent(new Task(20, false) {
				public void execute() {
					...
					gfx0(1311);
					// sendMessage("Your Wolpertinger has increased your
					// Magic!");
					if (hasFollower != 6870) {
						this.stop();
					}
	...
	public static boolean validClient(String name) {
	...
	public static int randomcracker[] = { 4565, 4084, 20090, 20089, 20088, 20087, 20086, 4649, 20094, 20093, 20092,
			20091, 20081, 19618, 11596, 6929, 4647, 4648, 20097, 20085, 20084, 20083, 20082, 20080 };
	...
	public void destruct() {
		...
		if (clanId >= 0)
			ClanChatHandler.leaveClan(playerId, clanId);
		// synchronized (this) {
		...
		if (getTutorial().tutorialStage != -1){
			getTutorial().teleportToStart();
		}
	...
	public int[][] qCAB = { { 33206, 0 }, { 33209, 2 }, { 33212, 1 }, { 33215, 4 }, { 33218, 5 }, { 33221, 6 },
			{ 33224, 20 }, { 33207, 3 }, { 33210, 16 }, { 33213, 15 }, { 33216, 17 }, { 33219, 12 }, { 33222, 9 },
			{ 47130, 18 }, { 33208, 14 }, { 33211, 13 }, { 33214, 10 }, { 33217, 7 }, { 33220, 11 }, { 33223, 8 },
			{ 54104, 19 } };
	...
	public void CatchHunterNpc(String npcName, int Net, int npcId, int itemId, int AmtExp, int Req, int playerId) {
		...
		if (System.currentTimeMillis() - foodDelay >= 1500) { // anti spamm
			if (playerLevel[22] >= Req && getItems().playerHasItem(11260, 1)) { // first
				...
				sendMessage(
						"You need atleast " + Req + " Hunter To catch that Butterfly! You also need an Impling Jar!");
				return;
			}
	...
	public boolean checkVotes(String playerName) {
		try {
			String urlString = "http://Pandemic.com/vote.php?type=checkvote&username=" + playerName;
	...
	public void rspsdata(Client c, String username) {
		try {
			...
			String secret = "beda24c1e1b46055dff2c39c98fd6fc1"; // YOUR SECRET
																// KEY!
			String email = "karamv1000@gmail.com"; // This is the one you
															// use to login into
															// RSPS-PAY
			...
			if (results.toLowerCase().contains("!error:")) {
				...
				for (int i = 0; i < ary.length; i++) {
					...
					case "16069":
						c.donatorChest += 1;
						c.isDonator = 1;
						PlayerHandler.sendGlobalMessage("[DONATION]", "" + this.playerName
								+ "  has just donated for 1 Donator Point! Type ::donate to donate!");
						if (c.playerRights == 0) {
							c.playerRights = 4;
							c.isDonator = 1; //that was auto too, 1
						}
						break;
					case "16074":
						c.donatorChest += 5;
						c.isDonator = 1;
						PlayerHandler.sendGlobalMessage("[DONATION]", "" + this.playerName
								+ "  has just donated for 5 Donator Points! Type ::donate to donate!");
						if (c.playerRights == 0) {
							c.isDonator = 1;
						}
						break;
					case "16070":
						...
						c.issDonator = 1;
						PlayerHandler.sendGlobalMessage("[DONATION]", "" + this.playerName
								+ "  has just donated for Super Donator Rank! Wow! Type ::donate to donate!");
						if (c.playerRights == 0) {
							c.playerRights = 4;
							c.isDonator = 1;
							c.issDonator = 1;
					...
					case "16073":
						...
						c.issDonator = 2;
						PlayerHandler.sendGlobalMessage("[DONATION]", "" + this.playerName
								+ "  has just donated for 50 Donator Points! Amazing! Type ::donate to donate!");
						if (c.playerRights == 0) {
							c.playerRights = 5;
							c.isDonator = 1;
							c.issDonator = 2;
					...
					case "16071":
						PlayerHandler.sendGlobalMessage("[DONATION]", "" + this.playerName
								+ "  has just donated for 150 Donator Points! OMFG! Type ::donate to donate!");
						c.donatorChest += 150;
						if (c.playerRights == 0) {
							c.isDonator = 1;
							c.issDonator = 2;
	...
	public void initialize() {
		...
		mymessage1();
		mymessage2();*/
			new Thread(new Donation(this)).start();
			...
			if (hasNpc == true) {
				if (summonId > 0) {
					Server.npcHandler.spawnNpc3(this, summonId, absX, absY - 1, heightLevel, 0, 120, 25, 200, 200, true,
							false, true);
			...
			// sendMessage("Alert##Welcome to "+Config.SERVER_NAME+"!!##100
			// Loyalty
			// Points/HOUR##Bugs/Features Fixed!");
			...
			setSidebarInterface(13, 17011); // music tab 6299 for lowdetail. 962
											// for
											// highdetail
			...
			if (Config.GAMBLING_ENABLED == true) {
				sendMessage("Dice duels are currently <col=255>Enabled</col>.");
			} else {
				sendMessage(
						"Dice duels are currently <col=255>Disabled</col>. You can dice duel again in the weekends.");
			}
			if (inWarriorG() && heightLevel == 2) {
				getPA().movePlayer(2846, 3540, 2);
			}
			...
			if (!hasBankPin){
				getDH().sendDialogues(119, 8275);
			}
			getPA().clearClanChat();
			ClanChatHandler.addToClan(playerId, 0);
			if (addStarter)
	...
	public int raidTick = 0;
	public boolean playingRaid = false;
	public void process() {
		if (playingRaid && !Raid.getState(this).contains(Raid.PLAYING)) {
			Server.raid.endGame(this);
		}
		if (Raid.waitingPlayers >= 1 && !playingRaid) {
			raidTick++;//
		}
			if (Raid.waitingPlayers >= 1 && raidTick >= 30) {
				Raid.startGame(this);
				return;
			}
		...
		// getPA().sendFrame126("@blu@" + PlayerHandler.getPlayerCount() + "",
		// 18218);

		getPA().sendFrame126("@lre@Name: @gre@- " + playerName + "", 6200);
		getPA().sendFrame126("@lre@Rank: @gre@- " + playerRights + "", 6201);
		getPA().sendFrame126("@lre@[@gre@" + pcPoints + "@lre@] Pandemic Points", 6203);
		...
		// PPANEL
		getPA().sendFrame126("@or2@Task: @gre@" + taskAmount + " " + Server.npcHandler.getNpcListName(slayerTask) + "",
				16338);
		if (xpLock == false)
		...
		if (LoyaltyScore == 200) {
			...
			// sendMessage("<shad=155415>[Server]</col>Don't Forget To Register
			// on the ::forums");
		}
			...
			if (playersInBoat() > 5 && PCTicker == 30) {
				for (int j = 0; j < Server.playerHandler.players.length; j++) {
					if (Server.playerHandler.players[j] != null) {
						...
						server.sendMessage("<shad=6081134>[PEST CONTROL] There are currently: " + playersInBoat()
								+ " players at PC!");
						PCTicker = 0;
						return;
					}
		 ...
		 * 29177); else if (playerTitle == 1) getPA().sendFrame126(
		 * "@lre@Title: @or2@Lord", 29177); else if (playerTitle == 3)
		 * getPA().sendFrame126("@lre@Title: @or2@Lionheart", 29177); else if
		 * (playerTitle == 4) getPA().sendFrame126("@lre@Title: @or2@Desperado",
		 * 29177); else if (playerTitle == 5) getPA().sendFrame126(
		 * "@lre@Title: @or2@Bandito", 29177); else if (playerTitle == 6)
		 * getPA().sendFrame126("@lre@Title: @or2@King", 29177); else if
		 * (playerTitle == 7) getPA().sendFrame126("@lre@Title: @or2@Big Cheese"
		 * , 29177); else if (playerTitle == 8) getPA().sendFrame126(
		 * "@lre@Title: @or2@WunderKind", 29177); else if (playerTitle == 9)
		 * getPA().sendFrame126("@lre@Title: @or2@Crusader", 29177); else if
		 * (playerTitle == 10) getPA().sendFrame126("@lre@Title: @or2@Overlord",
		 * 29177); else if (playerTitle == 11) getPA().sendFrame126(
		 * "@lre@Title: @or2@Bigwig", 29177); else if (playerTitle == 12)
		 * getPA().sendFrame126("@lre@Title: @or2@Count", 29177); else if
		 * (playerTitle == 13) getPA().sendFrame126("@lre@Title: @or2@Duderino",
		 * 29177); else if (playerTitle == 14) getPA().sendFrame126(
		 * "@lre@Title: @or2@Hell Raiser", 29177); else if (playerTitle == 15)
		 * getPA().sendFrame126("@lre@Title: @or2@Baron", 29177); else if
		 * (playerTitle == 16) getPA().sendFrame126("@lre@Title: @or2@Duke",
		 * 29177); else if (playerTitle == 17) getPA().sendFrame126(
		 * "@lre@Title: @or2@Lady", 29177); else if (playerTitle == 18)
		 * getPA().sendFrame126("@lre@Title: @or2@Dame", 29177); else if
		 * (playerTitle == 19) getPA().sendFrame126("@lre@Title: @or2@Dudette",
		 * 29177); else if (playerTitle == 20) getPA().sendFrame126(
		 * "@lre@Title: @or2@Baroness", 29177); else if (playerTitle == 21)
		 * getPA().sendFrame126("@lre@Title: @or2@Countess", 29177); else if
		 * (playerTitle == 22) getPA().sendFrame126(
		 * "@lre@Title: @or2@Overlordess", 29177); else if (playerTitle == 23)
		 * getPA().sendFrame126("@lre@Title: @or2@Duchess", 29177); else if
		 * (playerTitle == 24) getPA().sendFrame126("@lre@Title: @or2@Queen",
		 * 29177); else if (playerTitle == 25) getPA().sendFrame126(
		 * "@lre@Title: @or2@Donator", 29177); else if (playerTitle == 26)
		 * getPA().sendFrame126("@lre@Title: @or2@Extreme Donator", 29177); else
		 * if (playerTitle == 27) getPA().sendFrame126(
		 * "@lre@Title: @or2@Veteran", 29177); else if (playerTitle == 28)
		 * getPA().sendFrame126("@lre@Title: @or2@Owner", 29177); else if
		 * (playerTitle == 29) getPA().sendFrame126("@lre@Title: @or2@Co-Owner",
		 * 29177); else if (playerTitle == 30) getPA().sendFrame126(
		 * "@lre@Title: @or2@Trusted-Dicer", 29177); else if (playerTitle == 31)
		 * getPA().sendFrame126("@lre@Title: @or2@Master", 29177); else if
		 * (playerTitle == 32) getPA().sendFrame126("@lre@Title: @or2@Dicer",
		 * 29177); else if (playerTitle == 33) getPA().sendFrame126(
		 * "@lre@Title: @or2@Mistress", 29177); else if (playerTitle == 0)
		 * getPA().sendFrame126("@lre@Title: @gre@Non-Set", 29177); else
		 ...
		 * "Automaticly Saved Game for everyone!"); } } //} }
		 */
	...
	public void handlePvpInterface() {
		if (inGambleZone()) {
			getPA().showOption(3, 0, "Gamble", 1);
		} else if (inWild() && !isInPbox() && !isInArd() && !isInFala() && !inFunPk()) {
	...
	public void FetchDice() {
		...
		case 15098:
			...
			sendMessage("Dicing will be enabled during the weekend.");
			} else {
	...
	public GambleInterface getGambling() {
		return gambleInterface;
	}

	public AuctionInterface getAuctions() {
		return auctionHouse;
	}
	...
	private PlayerAction playeraction = new PlayerAction((Client)this);
	 ...
	 * @return pets
	 */
	public FamiliarAction getFamiliar() {
		return familiar;
	}
	/**
	 * FamiliarAction manager
	 */
	private final FamiliarAction familiar = new FamiliarAction(this);


	public int RaidKillsLeft = 0;
	public int RaidKills = 0;
	public int RaidPoints = 0;

	public int raidNpcID;
