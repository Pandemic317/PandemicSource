...
public class PlayerHandler {
	...
	public static void messageAllStaff(String text, boolean message) {
		for (Player p : PlayerHandler.players) {
			Client o = (Client) p;
			if (o != null) {
				if (o.playerRights == 1 || o.playerRights == 2 || o.playerRights == 3 || o.playerRights == 7) {
					if (message) {
						o.sendMessage(text);
					} else {
...
public static void sendGlobalMessage(String type, String text){
		
        for (int j = 0; j < PlayerHandler.players.length; j++) {
            if (PlayerHandler.players[j] != null) {
                Client c2 = (Client)PlayerHandler.players[j];
                text = Misc.ucFirst(Misc.optimizeText(text));
                String color = "col=800000000>";
                String color2 = "<shad><col=800000000>";
                if (type.equals("[Pandemic]"))
                    color = "<col=800000000>";
                if (type.equals("[Heaven]"))
                    color = "<col=800000000>";
                if (type.equals("[IRON Heaven]"))
                    color = "<col=800000000>";
                if (type.equals("[HC IRON Heaven]"))
                    color = "<col=800000000>";
                if (type.equals("[DONATION]"))
                	color = "<col=800000000>";
                if (type.equals("[VOTE]"))
                    color = "<col=20451204>";
                c2.sendMessage(color2 + type + color + text);
            }
        }
    }
			...
			try {
				if (players[i].disconnected && (System.currentTimeMillis() - players[i].logoutDelay > 10000 || players[i].properLogout || kickAllPlayers)) {
					...
					if (PlayerSave.saveGame(o)) {
						/*if (players[i].playerRights == 1 || players[i].playerRights == 2 || players[i].playerRights == 7) {
							players[i].staffLogin += players[i].staffLogin1 / 1000;
							String playerWrite = "The total of " + players[i].playerName + " is: "+ (System.currentTimeMillis() - players[i].staffLogin)+ " seconds.";
