...
public int strBonus;
	public int[][] slayerReqs = {{1648,5},{1612,15},{1643,45},{1618,50},{1624,65},{1610,75},{1613,80},{2783,90}, {3590, 92}, {3591, 82}, {3592, 72}, {9465, 82}, {9467, 72}};
	
	public boolean goodSlayer(int i) {
		if (Server.npcHandler.npcs[i].npcType == 1615 || Server.npcHandler.npcs[i].npcType == 2264) {
			if (NPCHandler.npcs[i].HP > 2500 && NPCHandler.npcs[i].HP < 3000 && c.playerEquipment[c.playerWeapon] == 13444) {
			NPCHandler.npcs[i].forceChat("Oh no, the Abyssal Tentacle!");
			c.sendMessage("The Abyssal Tentacle completely rekt the Abyssal Demon.");
			NPCHandler.npcs[i].HP -= NPCHandler.npcs[i].HP;
			return true;
			}
			}
		if (Server.npcHandler.npcs[i].npcType == 1615 && c.playerEquipment[c.playerCape] == 896) {
		...
		if (Server.npcHandler.npcs[i].npcType == 1615 && c.playerEquipment[c.playerRing] == 773) {
			c.getPA().movePlayer(2772, 9340, 0);
			c.sendMessage("<col=800000000>You teleport to the Owner Cape zone. You can not use your Ring here.");
			return false;
			}
		if (Server.npcHandler.npcs[i].npcType == 2264 && c.playerEquipment[c.playerCape] == 896) {
			c.getPA().movePlayer(2772, 9340, 0);
			c.sendMessage("<col=800000000>You teleport to the Owner Cape zone. You can not use your Cape here.");
			return false;
			}
		if (Server.npcHandler.npcs[i].npcType == 2264 && c.playerEquipment[c.playerRing] == 773) {
			c.getPA().movePlayer(2772, 9340, 0);
			c.sendMessage("<col=800000000>You teleport to the Owner Cape zone. You can not use your Ring here.");
			return false;
			}
	...
	*/
	...
	if (c.playerEquipment[c.playerWeapon] == 4056) {
	c.doubleHit = true;
	}
	...
	if (c.playerEquipment[c.playerWeapon] == 968) { //4743
	c.doubleHit = true;
	c.gfx0(311);
	}
	if (c.playerEquipment[c.playerWeapon] == 17877) { //Valgor Sword
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 17882) { //royal rapier
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 12311) { //Golden royal rapier
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 12309) { //Golden Katana
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 17880) { //4743
	c.doubleHit = true;
	c.gfx0(1638);
	}
	if (c.playerEquipment[c.playerWeapon] == 3892) { //4743
	c.gfx0(1977);
	}
	if (c.playerEquipment[c.playerWeapon] == 11650) { //Asgardian Greataxe
	c.doubleHit = true;
	c.gfx0(1621);
	}
	if (c.playerEquipment[c.playerWeapon] == 19058) { //Ragnarök Sword
	c.doubleHit = true;
	c.gfx0(1492);
	}
	...
	if (c.playerEquipment[c.playerWeapon] == 17875) {
	c.doubleHit = true;
	c.gfx0(453);
	}
	if (c.playerEquipment[c.playerWeapon] == 13453) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 13451) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 12315) {
	c.doubleHit = true;
	c.gfx0(453);
	}
	long weaponDelay;
	if (c.playerEquipment[c.playerWeapon] == 9992) {
	weaponDelay = System.currentTimeMillis();
	if (System.currentTimeMillis() - weaponDelay < 1500) {
		c.playerEquipment[3] = -1;
		c.gfx0(287);
		c.getPA().refreshPlayersInterface();
		Server.npcHandler.applyDamage(999999999);
		//Server.npcHandler.npcs[i].HP = 1;
	}
	}
	if (c.playerEquipment[c.playerWeapon] == 9899) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 9900) {
	c.doubleHit = true;
	}
	if (c.playerEquipment[c.playerWeapon] == 17859) {
	c.doubleHit = true;
	c.gfx0(2600);
	}
	if (c.playerEquipment[c.playerWeapon] == 17878) {
	c.doubleHit = true;
	c.gfx0(2600);
	}
	if (c.playerEquipment[c.playerWeapon] == 9906) {
	c.doubleHit = true;
	c.gfx0(343);
	}
	if (c.playerEquipment[c.playerWeapon] == 17876) {
	c.doubleHit = true;
	c.gfx0(453);
	}
	...
	if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 9245){
                c.sendMessage("You cant use bolts with magic shortbow!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 861 && c.playerEquipment[c.playerArrows] == 11212){
                c.sendMessage("You cant use dragon arrows with this bow!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 853 && c.playerEquipment[c.playerArrows] == 11212){
                c.sendMessage("You cant use dragon arrows with this bow!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 849 && c.playerEquipment[c.playerArrows] == 11212){
                c.sendMessage("You cant use dragon arrows with this bow!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 841 && c.playerEquipment[c.playerArrows] == 11212){
                c.sendMessage("You cant use dragon arrows with this bow!");
                return;
	}
	if(c.playerEquipment[c.playerWeapon] == 843 && c.playerEquipment[c.playerArrows] == 11212){
                c.sendMessage("You cant use dragon arrows with this bow!");
                return;
	}
...
//				c.sendMessage("This is "+Server.npcHandler.npcz[i]+" NPC.");
			...
			if(c.attackTimer <= 0) {
				...
				boolean usingCross = c.playerEquipment[c.playerWeapon] == 9185 && c.playerEquipment[c.playerWeapon] == 20155 && c.playerEquipment[c.playerWeapon] == 4640 && c.playerEquipment[c.playerWeapon] == 19477 && c.playerEquipment[c.playerWeapon] == 18357 && c.playerEquipment[c.playerWeapon] == 12926;
				c.bonusAttack = 0;
				c.rangeItemUsed = 0;
				c.projectileStage = 0;
				//c.SaveGame();
				...
				if (c.playerEquipment[c.playerWeapon] == 12926 && c.playerEquipment[c.playerArrows] >= 1 && !c.usingMagic) {
					c.sendMessage("You must un-equiped any ammunition the blow pipe does not require any");
					c.stopMovement();
					resetPlayerAttack();
					return;				
				}
				...
				if(usingBow && !usingOtherRangeWeapons && !c.usingMagic || usingCross) { // range hit delay					
					...
					fireProjectileNpc();
					NPCHandler.npcs[i].HP -= NPCHandler.npcs[i].HP / 20;
				}
...
c.degradeSHelm();
	...
	public void delayedHit(int i) { // npc hit delay
		if (Server.npcHandler.npcs[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (c.playerEquipment[c.playerWeapon] == 11639) {
					Server.npcHandler.npcs[i].gfx0(754);
					damage = Server.npcHandler.npcs[i].HP/5;
					//c.handleHitMask(c.playerLevel[3]/10);
					c.dealDamage(c.playerLevel[3]/10);
					if (c.playerLevel[4] < 120) {
		            c.getPA().addSkillXP(c.playerLevel[4] * 135, 4);
					}
					c.gfx0(754);					
				}*/
			...
			//c.getPA().sendFrame126("" + i + ":" + NPCHandler.npcs[i].HP + ":" + NPCHandler.npcs[i].MaxHP, 35000);
	...
	public void applyNpcMeleeDamage(int i, int damageMask, int damage) {
		...
		if(c.fightMode == 3) {
			...
			if (c.ironGambler >= 1) {
				c.getPA().addSkillXP((damage*Config.IRONMELEE_EXP_RATE/3), 0); 
				c.getPA().addSkillXP((damage*Config.IRONMELEE_EXP_RATE/3), 1);
				c.getPA().addSkillXP((damage*Config.IRONMELEE_EXP_RATE/3), 2); 				
				c.getPA().addSkillXP((damage*Config.IRONMELEE_EXP_RATE/3), 3);
				} else
			...
			c.getPA().addSkillXP((damage*Config.STARTER_MELEE_EXP_RATE/3), 3);
				...
				if (c.ironGambler >= 1) {
					c.getPA().addSkillXP((damage*Config.IRONMELEE_EXP_RATE), c.fightMode); 
					c.getPA().addSkillXP((damage*Config.IRONMELEE_EXP_RATE/3), 3);
				} else
	...
	**/
	...
	for (int ei : CustomItem.CustomItem) {
		if(c.playerEquipment[c.playerHat] == ei || c.playerEquipment[c.playerCape] == ei || c.playerEquipment[c.playerWeapon] == ei
				|| c.playerEquipment[c.playerLegs] == ei || c.playerEquipment[c.playerChest] == ei || c.playerEquipment[c.playerFeet] == ei ||
				c.playerEquipment[c.playerShield] == ei || c.playerEquipment[c.playerAmulet] == ei || c.playerEquipment[c.playerRing] == ei) {
			c.getPA().movePlayer(3333, 3334, 0);
		}
	}
	if (c.ironGambler > 0 && o.ironGambler == 0) {
		c.sendMessage("You can only attack Iron Gamblers!");
        return;
	}
	if (c.ironGambler == 0 && o.ironGambler > 0) {
		c.sendMessage("You can not attack an Iron Gambler!");
        return;
	}
...
return;
		...
		if (Server.playerHandler.players[i] != null) {
			...
			if(c.attackTimer <= 0) {
				...
				if(correctBowAndArrows() < c.playerEquipment[c.playerArrows] && Config.CORRECT_ARROWS && usingBow && !usingCrystalBow() && usingCross && c.playerEquipment[c.playerWeapon] != 12926 && !c.usingMagic) {
					c.sendMessage("You can't use "+c.getItems().getItemName(c.playerEquipment[c.playerArrows]).toLowerCase()+"s with a "+c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase()+".");
					c.stopMovement();
					resetPlayerAttack();
					return;
...
//			Server.npcHandler.npcs[i].CIcon = 2;
	...
	public void playerDelayedHit(int i) {
		...
		case 13867: // Zuriel's Staff
			if (c.npcID == 10156) {
				c.dealDamage(200);
				c.sendMessage("Ihlakhizan deals 200 damage!");
				c.gfx0(542);
			}
			damage *= .90;
...
public void fireProjectilePlayer() {
		if(c.oldPlayerIndex > 0) {
			if(Server.playerHandler.players[c.oldPlayerIndex] != null) {
				...
				int offX = (pY - oY)* -1;
				int offY = (pX - oX)* -1;	
				if(c.playerEquipment[c.playerWeapon] == 15241 || c.playerEquipment[c.playerWeapon] == 11639) {
					if(!c.specGfx) {
						c.gfx0(2138);
					}
...
//			case 20159:
			...
			if(c.specAmount >= 8) {
				c.specAmount -= 8;
				if (c.playerLevel[4] <= 125) {
	            c.getPA().addSkillXP(20500, 4);
				}
				c.getItems().addSpecialBar(weapon);
				return true;
			}
...
//			case 20159:
	...
	**/
	...
	public int getWepAnim(String weaponName) {
		...
		if(c.playerEquipment[c.playerWeapon] == 6605
		|| c.playerEquipment[c.playerWeapon] == 6607 
		|| c.playerEquipment[c.playerWeapon] == 6609
		|| c.playerEquipment[c.playerWeapon] == 6611
		|| c.playerEquipment[c.playerWeapon] == 6613) {
        return 2075;
		}
		if(c.playerEquipment[c.playerWeapon] == 19987) {
			switch(c.fightMode) {
				case 0: 
                       	 	return 2080;
				case 1: 
		...
		if(c.playerEquipment[c.playerWeapon] == 14007) { //Trickster gloves
                        	return 3678;
		}
	...
	**/
	...
	public int rangeMaxHit() {
        ...
        if (weapon == 4223) {//Cbow 1/10
            ...
            rangehit = 3;
            rangehit += c.playerLevel[4] / 7;
        } else if (weapon == 861) {//magic shortbow
            rangehit = 100;
            rangehit += c.playerLevel[4] / 7;
        } else if (Arrows == 4740 && weapon == 4743) {//Karils bow
            rangehit = 300;
            rangehit += c.playerLevel[4] / 7;
        } else if  (weapon == 806) {//bronze dart
            rangehit = 9000000;
            rangehit += c.playerLevel[4] / 7;
        ...
        } else if (weapon == 806) {//Bronze darts
            rangehit = 9000000;
            rangehit += c.playerLevel[4] / 11;
        } else if (Arrows == 4740 && weapon == 4937) {//BoltRacks
			rangehit = 525;
            rangehit += c.playerLevel[4] / 5;
        } else if (Arrows == 15243 && weapon == 15241) {//hand cannon
			rangehit = 6;
            rangehit += c.playerLevel[4] / 5;
        } else if (weapon == 11639) {//1handed hand cannon
			rangehit = 50;
            rangehit += c.playerLevel[4] / 10;
            c.getPA().addSkillXP(105, 4);
        } else if (weapon == 18357) {//c crossbow
			rangehit = 1000;
...
if (c.playerEquipment[c.playerWeapon] == 11639)
	return 2143;
		if (c.playerEquipment[c.playerWeapon] == 9185 || c.playerEquipment[c.playerWeapon] == 12926)
			return 27;
		if (c.playerEquipment[c.playerWeapon] == 20155)
			return 27;
		if (c.playerEquipment[c.playerWeapon] == 19477)
