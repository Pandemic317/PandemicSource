...
public class PanelSettings {
	...
	public void executeCommand(String cmd) {
		...
		if(cmd.startsWith("unban")) {
			System.out.println("Unban command has been run.");
			String[] args = cmd.split(" ");
			if(args.length == 2) {
				//Unban <username>
				File file = new File("./Data/Bans/UsersBanned.txt");
				if(file.exists()) {
					try {
						...
						while((inf = in.readLine()) != null) {
							if(inf.equalsIgnoreCase(args[1])) {
								// Don't want to write in the file so leave this blank.
							}else {
								out.write(inf);
							}
						}
					...
					System.out.println("Error unbanning the user. Failed to find file path.");
				}
			}
 		}
