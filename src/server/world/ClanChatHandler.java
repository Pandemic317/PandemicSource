...
public class ClanChatHandler {
	...
	public static Clan[] clans = new Clan[400];
	...
	public static void updateClanChat(int clanId) {
	...
	public static void addToClan(int playerId, int clanId) {
	...
	public static void leaveClan(int playerId, int clanId) {
		if (clanId < 0) {
			Client c = (Client)Server.playerHandler.players[playerId];
	...
	public static void destructClan(int clanId) {
		if (clanId < 0)
			return;
		for (int j = 0; j < clans[clanId].members.length; j++) {
			if (clanId < 0)
	...
	public static void messageToClan(String message, int clanId) {
		if (clanId < 0)
			return;
		for (int j = 0; j < clans[clanId].members.length; j++) {
			if (clans[clanId].members[j] < 0)
	...
	public void playerMessageToClan(int playerId, String message, int clanId) {
		...
		if (c.npcKills < 50) {
							c.sendMessage("You need atleast 50 NPC Kills before you can use this chat");
							return;
						}
				c.sendMessage("[<col=597105>Clan-Chat</col>] "+Misc.optimizeText(Server.playerHandler.players[playerId].playerName)+": <col=12654363>" + message);
